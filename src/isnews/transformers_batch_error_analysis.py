"""Анализ ошибочных предсказаний для пакетного transformers-инференса."""
...
from src.isnews.transformers_batch_inference_evaluation import (
    TransformersBatchInferenceEvaluationReport,
    _find_label_column,
)
...
class TransformersBatchErrorAnalysisError(ValueError):
    """Ошибка анализа неверно классифицированных строк."""


@dataclass(frozen=True)
class TransformersBatchErrorAnalysisReport:
...
@dataclass(frozen=True)
class TransformersBatchErrorAnalysisPaths:
...
@dataclass(frozen=True)
class TransformersBatchErrorAnalysisResult:
    """Возвращает таблицу ошибочных строк и пути к сохраненным артефактам."""

    misclassified_dataframe: pd.DataFrame
    report: TransformersBatchErrorAnalysisReport
    paths: TransformersBatchErrorAnalysisPaths
...
def _sanitize_name(name: str) -> str:
    ...
    return cleaned_name or "transformers_batch_error_analysis"
...
def analyze_transformers_batch_errors(
    batch_inference_result: TransformersBatchTextInferenceResult,
    evaluation_report: TransformersBatchInferenceEvaluationReport,
    *,
    ...
    if dataframe.empty:
        raise TransformersBatchErrorAnalysisError(
    ...
    paths = TransformersBatchErrorAnalysisPaths(
        misclassified_rows_path=_get_available_path(
            project_paths.error_analysis_reports_dir
            / f"{base_name}_transformers_misclassified.csv"
        ),
        report_path=_get_available_path(
            project_paths.error_analysis_reports_dir
            / f"{base_name}_transformers_error_analysis.json"
        ),
    )
    ...
    return TransformersBatchErrorAnalysisResult(
        misclassified_dataframe=misclassified_dataframe,
        report=report,
        paths=paths,
    )
